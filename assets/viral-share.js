...
(function(){
  ...
  function drawTeamCard(abbr,titleLine,metaLine){
    ...
    ctx.font='700 26px ui-monospace'; ctx.fillStyle='#666'; ctx.fillText('gridiron.dumbmodel.com/?team='+(abbr||'CHI'),48,860);
  ...
  function drawPastModernCard(opts){
    ...
    var insight = '48-d MTNN recall@10 0.977 \u00B7 PC1 paint\u2192perim PC2 load PC3 ball \u00B7 '+ (opts.streak?'streak '+opts.streak+' \u00B7 ':'') +'gridiron.dumbmodel.com/play';
    ...
    ctx.fillStyle='#111'; ctx.font='900 28px ui-sans-serif'; ctx.fillText('Play today: gridiron.dumbmodel.com/play \u2192',56,H-92);
  ...
  function drawPackCard(opts){
    ...
    var cta = 'Play pack: gridiron.dumbmodel.com/play?pack='+longCode.slice(0,28)+(longCode.length>28?'…':'');
  ...
  async function sharePastModern(opts){
    try{
      ...
      var text='Vector Gridiron Past→Modern P#'+opts.puzzleNum+' '+score+' '+grid+'\nPast '+opts.pastName+' '+opts.pastSeason+' → '+(opts.won? opts.guesses.slice(-1)[0].name+' 🎯' : 'modern twin?')+'\n'+(opts.streak?'🔥 streak '+opts.streak+' · ':'')+'12,966 as 3D map \ngridiron.dumbmodel.com/play?day='+opts.dayKey;
      if(navigator.canShare && navigator.canShare({files:[file]})){
        await navigator.share({title:'Vector Gridiron P#'+opts.puzzleNum,text:text,files:[file]});
        return true;
      }
      if(navigator.share){
  ...
  async function sharePack(opts){
    try{
      ...
      var baseOrigin = (location.origin||'https://gridiron.dumbmodel.com');
      ...
      var text='Vector Gridiron Pack Battle ('+(opts.size||'?')+') — '+solved+'/'+(opts.size||'?')+' in '+total+' guesses avg '+avg+' '+gridEmoji+battleTxt+'\n'+lines+'\nChallenge same pack: '+url;
      if(navigator.canShare && navigator.canShare({files:[file]})){
        await navigator.share({title:'Vector Gridiron Pack '+ (opts.size||'?'), text:text, files:[file]});
        return true;
      }
      if(navigator.share){
  ...
  async function shareUniverse(abbr,text,url,copiedEl){
    ...
    try{
      ...
      var file=new File([blob],'vector-gridiron-'+abbr+'.png',{type:'image/png'}); var shareText=text+'\n'+url;
      if(navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({title:'Vector Gridiron — '+abbr+' Universe',text:shareText,files:[file]}); if(copiedEl){copiedEl.style.display='block'; copiedEl.textContent='Shared!'; setTimeout(function(){copiedEl.style.display='none';},2500);} showToast('🌌 '+abbr+' universe shared'); return;}
      if(navigator.share){try{await navigator.share({title:'Vector Gridiron',text:shareText,url:url}); if(copiedEl){copiedEl.style.display='block'; setTimeout(function(){copiedEl.style.display='none';},2500);} showToast('Link shared — challenge friends'); return;}catch(e){}}
  ...
  function drawChimeraCard(puzzleNum,emojiRows,scoreText){
    ...
    ctx.fillStyle='#F0E442'; ctx.strokeStyle='#111'; ctx.lineWidth=4; roundedRect(ctx,36,860,W-72,72,18); ctx.fill(); ctx.stroke(); ctx.fillStyle='#111'; ctx.font='900 30px ui-sans-serif'; ctx.fillText('Play today: gridiron.dumbmodel.com/play',56,906);
    return c;
  }
  async function shareChimera(puzzleNum,emojiRows,scoreText,url){
    try{
      ...
      var file=new File([blob],'chimera-'+puzzleNum+'.png',{type:'image/png'}); var text='Vector Gridiron Chimera #'+puzzleNum+' — '+scoreText+'\n'+emojiRows+'\ngridiron.dumbmodel.com/play';
      if(navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({title:'Vector Gridiron #'+puzzleNum,text:text,files:[file]}); return true;}
  ...
  async function shareDailyCourt(opts){
    try{
      ...
      ctx.fillStyle='#111'; ctx.font='900 24px ui-sans-serif'; ctx.fillText('Play same court: gridiron.dumbmodel.com/play?day='+(opts.dayKey||''),56,H-64);
      ...
      var url=opts.url||('https://gridiron.dumbmodel.com/play?day='+(opts.dayKey||''));
      var text='Vector Gridiron Daily Court '+ (opts.dayKey||'')+' — '+opts.solved+'/5 in '+opts.totalGuesses+' guesses '+gridEmoji+'\n'+lines+'\nCan you beat my Starting 5? '+url;
