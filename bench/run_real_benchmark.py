...
# --------------------------------------------------------------------------- #
def train_mtnn_multitask(data: dict, epochs: int, patience: int = 15):
    """Train GridironMTNN (towers + TransformerFusion) with 3 regression heads.

    Returns (per-target test predictions dict, config dict).
    """
    import torch
    ...
    for ep in range(epochs):
        ...
        with torch.no_grad():
            ...
            vloss = float(lossf(vout, Yt[val_idx]))
        epochs_run = ep + 1
        ...
        if (ep + 1) % 10 == 0 or ep == 0:
            print(
                f"[mtnn] epoch {ep + 1:3d} train_mse={ep_loss / max(nb, 1):.4f} "
                f"val_mse={vloss:.4f} best={best_val:.4f}@{best_epoch} "
                f"({time.time() - t0:.0f}s)"
            )
    ...
    config = {
        ...
        "loss": "mean MSE over 3 z-scored targets (target stats fit on train rows only)",
        "batch_size": batch,
        "max_epochs": epochs,
        "epochs_run": epochs_run,
        "best_epoch": best_epoch,
        "best_val_mse_z": round(best_val, 6),
    ...
    print(f"[mtnn] done: best val_mse(z) {best_val:.4f} at epoch {best_epoch}; {config['params_total']} params")
    return preds, config
...
def main(argv: list[str] | None = None) -> int:
    ...
    preds, config = train_mtnn_multitask(data, epochs=args.epochs)
    ...
    for t in TARGETS:
        ...
        tasks[t] = build_task_for_target(
            ...
            extra_notes={
                ...
                "mtnn_training": json.dumps(config["dims"]) + f" seed={SEED} best_epoch={config['best_epoch']}",
            },
        )
