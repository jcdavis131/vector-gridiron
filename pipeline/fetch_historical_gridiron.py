...
def parse_covers_team_page(html: str) -> dict[int, float]:
    ...
    for year_str, ou_str in re.findall(
        r"<td[^>]*>\s*(20[0-2]\d|19\d{2})\s*</td>\s*.*?<\s*td[^>]*>\s*(\d{1,2}\.[05]|N/A)\s*</td>",
        search_area,
        re.S | re.I,
    ):
        if ou_str == "N/A":
...
def fetch_covers_nfl_win_totals(allow_gambling=False, single_team=None, force=False, offline=False, since=2000):
    ...
    if (
        not seasons
        and doc
        and all(isinstance(v, dict) for k, v in doc.items() if k not in ("built", "source", "coverage", "seasons"))
    ):
    ...
    for k, v in seasons.items():
        ...
        if isinstance(v, dict):
            seasons_norm[str(k)] = {
                str(team).upper(): float(val) for team, val in v.items() if isinstance(val, int | float)
            }
        else:
            seasons_norm[str(k)] = {}
    ...
    if offline:
        ...
        if not DEST.exists():
            doc["built"] = datetime.datetime.now(datetime.UTC).isoformat()
    ...
    if not allow_gambling:
        print(
            "[gridiron] Covers is gambling domain — pass --allow-gambling "
            "after user confirmation to enable live fetch.",
            flush=True,
        )
    ...
    if merged_any or not DEST.exists():
        doc["built"] = datetime.datetime.utcnow().isoformat() + "Z"
        doc["source"] = (
            "Covers SportsOddsHistory NFL team pages O/U + BetMGM manual (gated)"
            if allow_gambling
            else "offline stub — Covers gated"
        )
        full = sum(1 for vs in seasons_norm.values() if len(vs) >= 20)
        doc["coverage"] = (
            f"{full}/{len(seasons_norm)} >=20 teams, total entries {sum(len(v) for v in seasons_norm.values())}"
        )
        doc["seasons"] = seasons_norm
        DEST.parent.mkdir(parents=True, exist_ok=True)
        DEST.write_text(json.dumps(doc, indent=2), encoding="utf-8")
    ...
    print(
        f"seasons {len(seasons_norm)} full>=20 {sum(1 for v in seasons_norm.values() if len(v)>=20)} "
        f"total entries {sum(len(v) for v in seasons_norm.values())}"
    )
...
def fetch_draft_pedigree_stub(out_path=None, offline=False):
    ...
    stub = {
        ...
        "players": {},
        "note": "Gridiron analog to hoops draft pedigree: round/pick affects fantasy prior + forty affects "
        "speed score. Combine 40-yard dash ~4.22-4.84 WR/RB. Analogous to hoops 1.4MB player props.",
        "todo": [
