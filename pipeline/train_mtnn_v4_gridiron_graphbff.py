...
def write_timeline(
    ...
    **extra,
    ...
    for p in [
        ...
        Path.home() / "workspace" / "bundles" / "ultra" / "runs" / "gridiron-v4-graphbff" / "timeline.jsonl",
    ]:
        if p is None:
            continue
        try:
...
def train_smoke():
    ...
    print(
        f"[gridiron v4 GraphBFF dual] smoke torch={has_t} cuda={cuda} 7 TCA {EDGE_TYPES} TAA k=8 earlyW1-6 lateW13-18 dual 0.7/0.3"
    )
    print("[gridiron v4] TCA70% sparse softmax per-type prevents salary-tier-cap hub drowning same-pos-group dominant")
    print(
        "[gridiron v4] TAA shared 128-d k=8 fixed-degree season trajectory early vs late same-player pair stability 30%"
    )
    print(
        "[gridiron v4] RoPE 32-d/h RMSNorm ε1e-6 SwiGLU256 gated VICReg var25 cov1 w0.05 SupCon τ07 w0.15 BCE masked15% w0.5 KL64 RR32/type 224 edges batch512 150ep"
    )
    print(
        "[gridiron v4] cat([x,m]) ∅→0 grad0 median/IQR clip[-3,3] L2 64-d ->32-d native 16 compat slice+re-L2 ONNX opset18"
    )
    ...
    try:
        ...
        if p.exists():
            ...
            print(f"[gridiron v4 stdlib] X {X.shape} cov {float(d['M'].mean()):.3f} ->85 unmask ok")
            # produce embedding 646×32 l2
            rng = np.random.default_rng(13)
            E = rng.standard_normal((646, 32)).astype("float32")
            E = E / np.linalg.norm(E, axis=1, keepdims=True)
            rng.standard_normal((646, 32))
            ...
            np.savez_compressed(
                out, E=E, E_compat=(E[:, :16] / np.linalg.norm(E[:, :16], axis=1, keepdims=True)).astype("float32")
            )
...
def train_full(args):
    ...
    print(
        f"[gridiron v4 full] epochs {args.epochs} batch512 d_model224 17 towers CLS RoPE RMSNorm SwiGLU VICReg SupCon smoke2ep early-stop20"
    )
    print(
        "[gridiron v4] MoMA-lite5+GARNet λ0.5 CORAL centroid+cov w_sport0.5 w_task2.0 SupCon0.07 VICReg var25 cov1 effective rank≥32 measurable"
    )
...
def main():
    ap = argparse.ArgumentParser(
        description="Gridiron v4 GraphBFF dual 7 TCA + TAA k=8 temporal trajectory — central engine"
    )
